...
TEST_CASE("Block", "[storage]")
{
    ...
    {
      {
        ...
        REQUIRE(dst.copy(src) == HeError::None);
        checkHostMemory(dst);
      }
      {
        struct CopyTest
        ...
        Block hm;
        REQUIRE(hm.resize(sizeof(CopyTest)) == HeError::None);
        CopyTest ct;
        ct.a = 3;
        ct.b = 6;
        REQUIRE(hm.copy(&ct) == HeError::None);
  ...
  SECTION("resize")
  {
    SECTION("host")
    {
      ...
      REQUIRE(hm.resize({64, 2}, 0) == HeError::None);
      writeHostMemory(hm);
      REQUIRE(hm.sizeInBytes() == 128);
      REQUIRE(checkHostMemory(hm));
      REQUIRE(hm.resize({32, 4, 2}, 0) == HeError::None);
      writeHostMemory(hm);
    ...
    SECTION("device")
    {
      ...
      REQUIRE(dm.resize(256) == HeError::None);
...
TEST_CASE("AOS", "[storage][aos]")
{
  ...
  SECTION("Sanity Checks")
  {
    ...
    REQUIRE(fields[2].component_count == 1);
    REQUIRE(fields[2].type == DataType::I32);
    REQUIRE(aos.resize(40) == HeError::None);
    REQUIRE(aos.size() == 40);
  ...
  SECTION("change description")
  {
    ...
    // check fields
    AoS aos;
    REQUIRE(aos.setLayout(desc) == HeError::None);
    auto fields = aos.layout().fields();
    REQUIRE(fields.size() == 3);
    REQUIRE(fields[0].name == "geo::vec3");
    REQUIRE(fields[0].size == sizeof(geo::vec3));

...
TEST_CASE("Grid1", "[numeric][grid]") {
  ...
  SECTION("ConstGridAccessor") {
    ...
    auto cf = [](const Grid1<float> &g, float dx, const
std::function<float(real_t)> &f) {
      // cell info
      REQUIRE(g.accessor().cellRegion(4).extends() == Approx(dx).margin(1e-8));
      REQUIRE(g.accessor().cellIndex(5.5 * dx) == 5);
  ...
  SECTION("Linear interpolation") {
    SECTION("constant function") {
      ...
      g = 4.f;
      auto acc = g.accessor(AddressMode::CLAMP_TO_EDGE,
interpolationMode::LINEAR); REQUIRE(acc(0.5) == Approx(4.f).margin(1e-6));
...
TEST_CASE("Grid2", "[numeric][grid]") {
  ...
  SECTION("Apply") {
    ...
    for (auto g : grid.accessor())
      REQUIRE(g.value == Approx(g.worldPosition().x * 100 +
g.worldPosition().y)); grid = [](const hermes::point2 &p) -> f32 { return p.y *
100 + p.x; }; for (auto g : grid.accessor()) REQUIRE(g.value ==
Approx(g.worldPosition().y * 100 + g.worldPosition().x));
  }//
  SECTION("GridIterator") {
  ...
  SECTION("ConstGridAccessor") {
    ...
    auto cf = [](const Grid2<float> &g, float dx, const
std::function<float(point2)> &f) {
      // cell info
      REQUIRE(g.accessor().cellRegion(index2(4, 4)).extends().x ==
          Approx(dx).margin(1e-8));
      REQUIRE(g.accessor().cellRegion(index2(4, 4)).extends().y ==
          Approx(dx).margin(1e-8));
      REQUIRE(g.accessor().cellIndex(point2(5.5 * dx, 7.8 * dx)) == index2(5,
7)); REQUIRE(g.accessor().cellPosition(point2(3.1 * dx, 8.7 * dx)).x ==
          Approx(0.1).margin(1e-6));
      REQUIRE(g.accessor().cellPosition(point2(3.1 * dx, 8.7 * dx)).y ==
          Approx(0.7).margin(1e-6));
      SECTION("CLAMP_TO_EDGE + LINEAR INTERPOLATION") {
  ...
  SECTION("2d") {
    SECTION("sanity") {
      ...
      for (index2 ij : Index2Range<i32>(A.grid_size())) {
        REQUIRE(
            ans[idx] ==
                (iacc.stores(ij.left()) ? iacc[ij.left()] : 0) * A(ij,
ij.left()) + (iacc[ij.down()] ? iacc[ij.down()] : 0) * A(ij, ij.down()) +
                    (iacc.stores(ij) ? iacc[ij] : 0) * A(ij, ij) +
                    (iacc.stores(ij.right()) ? iacc[ij.right()] : 0) *
                        A(ij, ij.right()) +
                    (iacc.stores(ij.up()) ? iacc[ij.up()] : 0) * A(ij,
ij.up())); idx++;
      }
      Vector<f32> r = A * x;
...
}; auto grad = DiffOps::grad(field.constAccessor()); auto acc = grad.accessor();
      for (int i = 0; i < 100000; ++i) {
...
TEST_CASE("interpolation", "[numeric][interpolation]") {
  SECTION("monotonicCubic") {
    { // 1D test
      ...
      for (float s = 0.0; s <= 1.0; s += 0.01) {
        REQUIRE(ponos::monotonicCubicinterpolate(f(-0.1), f(0.0), f(0.1),
f(0.2), s) == Approx(f(s * 0.1)).margin(0.1 * 0.1));
      }
      for (float s = 0.0; s <= 1.0; s += 0.01)
        REQUIRE(
            ponos::monotonicCubicinterpolate(f(-0.01), f(0.0), f(0.01), f(0.02),
s) == Approx(f(s * 0.01)).margin(0.01 * 0.01));
    }
  }
}
